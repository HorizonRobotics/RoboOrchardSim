...
"""Resolver-backed definition for the open benchmark."""
...
_CONFIG_PATH = Path(__file__).resolve().parent / "configs/open.yaml"


@register_task
class OpenTaskDefinition(TaskDefinition):
    """Build the single-target open benchmark."""
    ...
    def build(
        ...
        """Build an open environment from resolved assets."""
        ...
        from robo_orchard_sim.orchard_env.task_templates.articulated.open_task import (  # noqa: E501
            JointOpenTask,
        )

        if resolver is None:
            raise ValueError(
                "OpenTaskDefinition.build() requires an AssetResolver."
            )
        asset_configs = cls.resolve_asset_configs(config_path=config_path)
        if asset_configs is None:
            raise ValueError(
                "OpenTaskDefinition needs asset_configs in "
