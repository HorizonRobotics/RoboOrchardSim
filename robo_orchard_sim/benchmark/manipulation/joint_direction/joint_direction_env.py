...
"""Resolver-backed definition for the joint-direction benchmark."""
...
_CONFIG_PATH = Path(__file__).resolve().parent / "configs/joint_direction.yaml"


@register_task
class JointDirectionTaskDefinition(TaskDefinition):
    """Build the single-target cabinet joint-direction benchmark."""

    namespace = "joint_direction"
    ...
    def build(
        ...
        """Build a joint-direction environment from resolved assets."""
        ...
        if resolver is None:
            raise ValueError(
                "JointDirectionTaskDefinition.build() requires an "
                "AssetResolver."
            )
        asset_configs = cls.resolve_asset_configs(config_path=config_path)
        if asset_configs is None:
            raise ValueError(
                "JointDirectionTaskDefinition needs asset_configs in "
        ...
        return OrchardEnv(
            ...
            task=JointDirectionTask(
                assets=TaskAssets.from_resolved(resolved),
                params=JointDirectionTaskParams(
...
__all__ = ["JointDirectionTaskDefinition"]
