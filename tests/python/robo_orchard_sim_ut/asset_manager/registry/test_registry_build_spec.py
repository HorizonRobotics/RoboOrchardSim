...
def test_build_spec_name_override(mini_asset_root: Path):
    reg = AssetRegistry(str(mini_asset_root))
    meta = reg.get_by_asset_id("apple_001")
    spec = reg.build_spec(meta, name="pick_object")
    assert spec.name == "pick_object"


def test_build_spec_scale_override(mini_asset_root: Path):
    reg = AssetRegistry(str(mini_asset_root))
    meta = reg.get_by_asset_id("apple_001")
    spec = reg.build_spec(meta, scale=(2.0, 2.0, 2.0))
    assert spec.scale == (2.0, 2.0, 2.0)
